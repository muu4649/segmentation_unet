...
from util.model_keras import build_model, original_loss,original_accuracy,auc
...
def load_dataset(train_rate):
    loader = ld.Loader(dir_original="data_set/VOCdevkit/VOC2013/512",
                       dir_segmented="data_set/VOCdevkit/VOC2013/512/lbl")
...
class SavePredictionCallback(keras.callbacks.Callback):
     ...
     def on_epoch_end(self, epoch, logs={}):
         if epoch % 5== 0:
             ...
             outputs_train = self.model.predict(self.trains.images_original[idx_train].reshape(1,512,512,3), verbose=0)
             outputs_test = self.model.predict(self.tests.images_original[idx_test].reshape(1,512,512,3), verbose=0)
...
def train(args):
    ...
    callbacks_list =[
        ...
        ModelCheckpoint(filepath='./pits_model/model_{epoch:02d}_{val_loss:.2f}.h5',monitor='loss', save_best_only=True, verbose=1, mode='auto')
        ]
...
def get_parser():
    parser = argparse.ArgumentParser(
        ...
        usage='python main_keras_Unet.py',
    ...
    parser.add_argument('-t', '--trainrate', type=float, default=0.9, help='Training rate')
