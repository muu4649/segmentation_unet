...
def build_model(output_class_num, size=None, l2_reg=None):
    ...
    conv1_1 = base_conv(inputs, filters=64, l2_reg_scale=l2_reg)

    conv1_2 = base_conv(conv1_1, filters=64, l2_reg_scale=l2_reg)
    pool1 = layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2))(conv1_2)
    
    # 1/2, 1/2, 64
    conv2_1 = base_conv(pool1, filters=128, l2_reg_scale=l2_reg)
    conv2_2 = base_conv(conv2_1, filters=128, l2_reg_scale=l2_reg)
    pool2 = layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2))(conv2_2)
    
    # 1/4, 1/4, 128
    conv3_1 = base_conv(pool2, filters=256, l2_reg_scale=l2_reg)
    conv3_2 = base_conv(conv3_1, filters=256, l2_reg_scale=l2_reg)
    pool3 = layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2))(conv3_2)
    
    # 1/8, 1/8, 256
    conv4_1 = base_conv(pool3, filters=512, l2_reg_scale=l2_reg)
    conv4_2 = base_conv(conv4_1, filters=512, l2_reg_scale=l2_reg)
    pool4 = layers.MaxPooling2D(pool_size=(2, 2), strides=(2, 2))(conv4_2)

    # 1/16, 1/16, 512
    conv5_1 = base_conv(pool4, filters=1024, l2_reg_scale=l2_reg)
    conv5_2 = base_conv(conv5_1, filters=1024, l2_reg_scale=l2_reg)
    concated1 = layers.Concatenate()([conv_transpose(conv5_2, filters=512, l2_reg_scale=l2_reg), conv4_2])

    conv_up1_1 = base_conv(concated1, filters=512, l2_reg_scale=l2_reg)
    conv_up1_2 = base_conv(conv_up1_1, filters=512, l2_reg_scale=l2_reg)
    concated2 = layers.Concatenate()([conv_transpose(conv_up1_2, filters=256, l2_reg_scale=l2_reg), conv3_2])

    conv_up2_1 = base_conv(concated2, filters=256, l2_reg_scale=l2_reg)
    conv_up2_2 = base_conv(conv_up2_1, filters=256, l2_reg_scale=l2_reg)
    concated3 = layers.Concatenate()([conv_transpose(conv_up2_2, filters=128, l2_reg_scale=l2_reg), conv2_2])

    conv_up3_1 = base_conv(concated3, filters=128, l2_reg_scale=l2_reg)
    conv_up3_2 = base_conv(conv_up3_1, filters=128, l2_reg_scale=l2_reg)
...
def base_conv(inputs, filters, kernel_size=(3, 3), activation='relu', l2_reg_scale=None):
    reg = tf.keras.regularizers.l2(l2_reg_scale) if l2_reg_scale is not None else None
