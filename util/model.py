...
class UNet:
    ...
    def create_model(size, l2_reg):
        ...
        # 1/8, 1/8, 256
        conv4_1 = UNet.conv(pool3, filters=64, l2_reg_scale=l2_reg, batchnorm_istraining=is_training)
        conv4_2 = UNet.conv(conv4_1, filters=64, l2_reg_scale=l2_reg, batchnorm_istraining=is_training)
        pool4 = UNet.pool(conv4_2)

        # 1/16, 1/16, 512
        conv5_1 = UNet.conv(pool4, filters=128, l2_reg_scale=l2_reg)
        conv5_2 = UNet.conv(conv5_1, filters=128, l2_reg_scale=l2_reg)
        concated1 = tf.concat([UNet.conv_transpose(conv5_2, filters=64, l2_reg_scale=l2_reg), conv4_2], axis=3)

        conv_up1_1 = UNet.conv(concated1, filters=64, l2_reg_scale=l2_reg)
        conv_up1_2 = UNet.conv(conv_up1_1, filters=64, l2_reg_scale=l2_reg)
        ...
        conv_up2_2 = UNet.conv(conv_up2_1, filters=32, l2_reg_scale=l2_reg)
        ...
        conv_up4_1 = UNet.conv(concated4, filters=8, l2_reg_scale=l2_reg)
        conv_up4_2 = UNet.conv(conv_up4_1, filters=8, l2_reg_scale=l2_reg)
    ...
    @staticmethod
    def create_model_original(size, l2_reg):
        inputs = tf.placeholder(tf.float32, [None, size[0], size[1], 3])
        teacher = tf.placeholder(tf.float32, [None, size[0], size[1], len(ld.DataSet.CATEGORY)])
        is_training = tf.placeholder(tf.bool)
